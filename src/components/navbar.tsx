...
const Navbar = () => {
  ...
  // let { login, setLogin }: any = useContext(contextApi);
  // console.log(login);
  // console.log(13);
  return (
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div className="flex lg:flex-1">
          <Link href="/" className="-m-1.5 p-1.5">
            ...
            <Image className="h-8 w-auto" src="/imdb.svg"  alt="dd"
                  width={100}
                  height={50} />
          </Link>
        </div>
        ...
        <Popover.Group className="hidden lg:flex lg:gap-x-12">
          <Popover className="relative">
            ...
            >
              <Popover.Panel className="absolute -left-8 top-full z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl bg-gray-800 shadow-lg ring-1 ring-white/5">
                <div className="p-4">
                  {products.map((item) => (
                    <div
                      key={item.name}
                    ...
                    >
                      <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-white group-hover:bg-gray-800">
                        <item.icon className="h-6 w-6 text-gray-800 group-hover:text-indigo-400" aria-hidden="true" />
                      </div>
          ...
          <Link href="/movies/addmovie" className="text-sm font-semibold leading-6 text-white">
            Add Movie
          </Link>
        </Popover.Group>
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          <Link href={login.menuAddress} className="text-sm font-semibold leading-6 text-white">
            {login.menuTitle} <span aria-hidden="true">&rarr;</span>
          </Link>
        </div>
      </nav>
      <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        <div className="fixed inset-0 z-10" />
        <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-gray-800 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-white/10">
